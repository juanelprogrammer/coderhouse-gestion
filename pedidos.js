...
const mostrarArray = (array) => {
  ...
  array.forEach((pedido) => {
    ...
    tr.innerHTML += `
        ...
        <button title="Editar" onclick="editarById(${pedido.id})">
...
const editarById = (pedidoId) => {
  const pedido = pedidos.find((ped) => ped.id == pedidoId)
  mostrarModal()
  
  //renderizo y agrego los productos del pedido a la tabla para poder editarlos
  let prodPedidos = []
  for (const prod of pedido.productos) prodPedidos.push(prod)
  modalContainer.innerHTML = render.formEditarPedido(pedidoId, prodPedidos)
  mostrarProductosPedido(prodPedidos)
  ...
  inputProv.addEventListener('input', () => {
    ...
    options.forEach((option) => {
      option.addEventListener('click', () => {
        ...
        inputProv.value = option.innerText
        inputProv.readOnly = true
        //cambio el nombre del proveedor en pantalla
        const pProv = document.querySelector('#proveedor-nombre')
        pProv.innerHTML = proveedor.nombre
        //si el proveedor cambio tengo que borrar los productos que estan pedidos
       let prodPedidos = []
       mostrarProductosPedido(prodPedidos)
  ...
  inputProv.ondblclick = () => {
    ...
    options.forEach((option) => {
      option.addEventListener('click', () => {
        ...
        //si el proveedor cambio tengo que borrar los productos que estan pedidos
       if(pedido.proveedor != option.value) {
        let prodPedidos = []
        mostrarProductosPedido(prodPedidos)}
      })
    })
